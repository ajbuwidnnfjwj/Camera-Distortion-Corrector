...
if __name__ == '__main__':
    distorted_video = cv.VideoCapture("./chessboard.MOV")
    board_pattern = (10, 7)
    ...
    while True:
        ...
        out.open('./distortion correction.avi', cv.VideoWriter_fourcc(*'XVID'), 
                distorted_video.get(cv.CAP_PROP_FPS), (img_corrected.shape[1], img_corrected.shape[0]))
        key = cv.waitKey(10)
        if key == 27:
            break